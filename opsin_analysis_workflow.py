...
# Error Analysis
from projects.opsin_analysis.error_analysis import calculate_structure_errors
...
from projects.opsin_analysis.helix_analysis import align_to_reference_and_annotate_helices

# GRN Assignment
from projects.opsin_analysis.assign_grns import align_and_assign_grn
...
def run_opsin_analysis_workflow(output_dir=None,
    ...
    # Try different paths for property file
    property_paths = [
        property_file,  # User-provided path first
        os.path.join(output_dir, "property", "mo_exp.csv"),  # In output dir
        os.path.join(os.path.dirname(__file__), "property", "mo_exp.csv"),  # Default path
        "projects/opsin_analysis/property/mo_exp.csv"  # Legacy path
    ]
    
    # Filter out None values
    property_paths = [p for p in property_paths if p is not None]
    
    # Try each path
    found_property_file = False
    for property_path in property_paths:
        if os.path.exists(property_path):
            ...
            found_property_file = True
            break
    
    if not found_property_file:
        print(f"No property file found in any of these locations: {property_paths}")
    ...
    if comparison_data is None:
        ...
        comparison_data = compare_structures(data, output_dir, visualize=visualize)
        data.update(comparison_data)
    ...
    if grn_data is None:
        ...
        grn_data = align_and_assign_grn(data, output_dir, visualize=visualize)
