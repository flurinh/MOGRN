...
def setup_registry_and_datasets(cp, paths, user_data_root, project_root):
    ...
    if legacy_dataset_path.exists():
        ...
        ref_data_dir = Path(paths.ref_data_root) if paths.ref_data_root else Path(
            project_root) / "protos" / "src" / "protos" / "data"
        ...
        try:
            cp.__init__(
                ...
                data_root=user_data_root,  # Pass the same user_data_root to processor
...
def setup_processor(user_data_root):
    ...
    # Initialize ProtosPaths for consistent path resolution
    ref_data_root = None  # Let ProtosPaths find the reference data root

    # Create an instance of ProtosPaths
    paths = ProtosPaths(
